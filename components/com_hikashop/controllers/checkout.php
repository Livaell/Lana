...
class checkoutController extends hikashopController {
	...
	function before_payment($directCall=false){
		...
		if(!empty($payment_method) && @$cart->payment->payment_params->payment_percentage != 0){
			...
			$cart->payment->payment_price = ($price_all * (float)@$cart->payment->payment_params->payment_percentage / 100) + $rates[$payment_id]->payment_price;
		}
	...
	function _readPayment(){
		...
		if(count($payment)>1){
			...
			if(($old_payment_id!=$payment_id || $old_payment_method!=$payment) && ($this->_getStep('cart',(int)$this->previous)===(int)$this->previous || $this->_getStep('confirm',(int)$this->previous)===(int)$this->previous)){
				return false;
			}
			return true;
		}else{
