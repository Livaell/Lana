...
function displayRegistration(el)
{
	...
	if(value=="login" && checked==true) {
		...
		document.getElementById("hika_registration_type").innerHTML="'.JText::_('HIKA_REGISTRATION').'";
		document.getElementById("hikashop_register_form_button").value="'.JText::_('HIKA_REGISTER').'";
		if(value==0)
		{
		...
		if(login_div) login_div.className="hikashop_hidden_checkout";
		if(registration_div) registration_div.className="";
		document.getElementById("hika_registration_type").innerHTML="'.JText::_('GUEST').'";
		document.getElementById("hikashop_register_form_button").value="'.JText::_('HIKA_NEXT').'";
	...
	function shipping() {
		...
		if(empty($usable_rates)) {
			...
			if($auto_select_default && empty($shipping_id) && count($usable_rates)) {
				...
				$currencyClass->processShippings($order->shipping);
