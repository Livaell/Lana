...
class JFormFieldArticle extends JFormField
{
	...
	protected function getInput()
	{
        ...
        if($joomla >= '3.0') {
    		...
    		$html[] = '<input class="input-medium" type="text" id="'.$id.'" value="'.$title.'" disabled="disabled" size="25" />';
    		$html[] = '<a class="modal btn"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 800, y: 450}}"><i class="icon-file"></i> '.JText::_('MOD_JUNEWS_ARTICLE').'</a>';
    		$html[] = "<a class=\"btn\" onclick=\"document.id('".$id."').value='".JText::_("MOD_JUNEWS_ARTICLE_INPUT")."'; document.id('".$id."').fireEvent('change'); return false;\" href=\"#\" title=\"Очистити\"><i class=\"icon-remove\"></i> ".JText::_("JCLEAR")."</a>";
    		$html[] = '</span';
