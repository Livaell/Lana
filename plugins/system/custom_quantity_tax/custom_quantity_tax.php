	...
	function quantityPrice(&$price,$quantity){
		if($quantity>0){
			...
			if(isset($price->price_orig_value_with_tax)){
				...
				$price->price_orig_value_with_tax=$currencyHelper->getTaxedPrice($price->unit_price->price_orig_value*$quantity,hikashop_getZone(),$this->product->product_tax_id,$rounding);
			}
			...
			if(isset($price->price_value_without_discount_with_tax)){
				...
				$price->price_value_without_discount_with_tax=$currencyHelper->getTaxedPrice($price->unit_price->price_value_without_discount*$quantity,hikashop_getZone(),$this->product->product_tax_id,$rounding);
			}

			if(isset($price->price_value_with_tax)){
				$price->unit_price->price_value_with_tax=round($price->price_value_with_tax,$rounding);

				$price->price_value_with_tax=$currencyHelper->getTaxedPrice($price->unit_price->price_value*$quantity,hikashop_getZone(),$this->product->product_tax_id,$rounding);
			}
			...
			if(isset($price->taxes)){
				...
				foreach($price->taxes as $k => $tax){
					...
					$price->taxes[$k]->tax_amount = $currencyHelper->getTaxedPrice($price->unit_price->price_value*$quantity,hikashop_getZone(),$this->product->product_tax_id,7)-$price->unit_price->price_value*$quantity;
				}
			}
			if(isset($price->taxes_without_discount)){
				...
				foreach($price->taxes_without_discount as $k => $tax){
					...
					$price->taxes_without_discount[$k]->tax_amount = $currencyHelper->getTaxedPrice($price->unit_price->price_value_without_discount*$quantity,hikashop_getZone(),$this->product->product_tax_id,7)-$price->unit_price->price_value_without_discount*$quantity;
				}
			}
