...
class plgHikashoppaymentbf_rbsbusinessgateway extends hikashopPaymentPlugin
{
	...
	function onPaymentDisplay(&$order,&$methods,&$usable_methods){
		if(!$this->isShippingValid(@$order->shipping))
			return true;
		$this->user = hikashop::loadUser(true);
	...
	function onPaymentNotification(&$statuses){
		...
		if ($hostError > 0) {
			$email = new stdClass();
			$email->subject = JText::sprintf('NOTIFICATION_REFUSED_FOR_THE_ORDER','Worldpay Business Gateway').' '.JText::sprintf('IP_NOT_VALID',hikashop::encode($dbOrder));
