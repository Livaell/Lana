...
class plgHikashoppaymentAuthorize extends hikashopPaymentPlugin
{
	...
	function _loadStandardVars(&$order){
		...
		if(!empty($order->cart->coupon) && @$order->cart->coupon->discount_value>0){
			$vars["x_line_item"][]='coupon<|>'.JText::_('HIKASHOP_COUPON').'<|><|>1<|>'.round($order->cart->coupon->discount_value,(int)$this->currency->currency_locale['int_frac_digits']).'<|>N';
		}
	...
	function onAfterOrderConfirm(&$order,&$methods,$method_id){
		...
		if(@$this->payment_params->api=='aim'){
			...
			$vars["x_fp_hash"] = hash_hmac("md5", $vars["x_login"] . "^" . $vars["x_fp_sequence"] . "^" . $vars["x_fp_timestamp"] . "^" . $vars["x_amount"] . "^", $this->payment_params->transaction_key);
