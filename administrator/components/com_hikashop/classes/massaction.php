				...
				foreach($element as $key=>$charac){
					if($key === 'characteristic'){
						foreach($charac as $elem){
							if(isset($elem->characteristic_parent_id) && $elem->characteristic_parent_id === '0'){
	...
	function _loadResults($table_name,$switch_table,$ids,$columns=null){
		...
		switch($table_name){
			case 'product':
				switch($switch_table){
					case 'price':
						$query = 'SELECT '.implode(',',$columns).', price_product_id, price_id';
			...
			foreach($params->elements as $k1=>$element){
				...
				foreach($params->action as $key=>$table){
					foreach($table as $column){
						$find = false;
						$square = '';
						if(isset($element->$column) && ($key===$k1 || $key===$params->table)){
							$square .= $massaction->displayByType($params->types,$element,$column);
							$find = true;
						}else{
							$r = array();
							foreach($element as $k=>$elem){
								...
								foreach($elem as $data){
									...
									if($k != $key){continue;}
									$r[] = $massaction->displayByType($params->types,$data,$column);
									$find = true;
								}
							}
