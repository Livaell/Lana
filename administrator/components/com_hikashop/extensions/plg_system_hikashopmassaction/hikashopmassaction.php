		...
		foreach($loadedData->massaction_filters as $key => &$value) {
			...
			if(!empty($tables)){
				...
				foreach($loadedData->massaction_filters as $key => &$value) {
					...
					$output = '<div id="'.$table->table.'filter__num__limit">'.JText::_('HIKA_START').' : <input type="text" name="filter['.$table->table.']['.$key.'][limit][start]" value="'.$value->data['start'].'" /> '.JText::_('VALUE').' : <input type="text" name="filter['.$table->table.']['.$key.'][limit][value]" value="'.$value->data['value'].'"/>'.'</div>';
					$filters_html[$value->name] = $massactionClass->initDefaultDiv($value, $key, $type, $table->table, $loadedData, $output);
				}
...
<?php
	...
	function onHikashopCronTrigger(&$messages){
		...
		$periods = array('hours','days','weeks','months','years');
		$massactionClass = hikashop_get('class.massaction');
