...
class plgHikashoppaymentPrivat24 extends JPlugin {

	var $accepted_currencies = array('UAH', 'EUR', 'USD');
	var $debugData = array();
	var $payment_name = 'privat24';
	var $payment_name_up = 'Privat24';
	var $payment_url = 'https://api.privatbank.ua/p24api/ishop';
	...
	function onAfterOrderConfirm(&$order, &$methods, $method_id) {
		...
		$vars = array(
			...
			'merchant' => $method->payment_params->merchant_id,
			'order' => $order->order_id,
			'details' => 'Оплата заказа № '.$order->order_id,
			'pay_way' => 'privat24',
			'ext_details' => '',
	...
	function onPaymentNotification(&$statuses) {
		...
		parse_str($vars['payment'], $payment_data);

		$secret_word = $element->payment_params->secret_word;
		$sign = sha1(md5($vars['payment'].$secret_word));

		if ($element->payment_params->debug)
			echo print_r($vars, true)."\n\n\n";

		$orderClass = hikashop_get('class.order');
		$dbOrder = $orderClass->get((int)$payment_data['order']);

		if (empty($dbOrder)) {
			echo 'Could not load any order for your notification '.$payment_data['order'];
			return false;
		}
		...
		$order->history->history_amount = $payment_data['amt'];
		...
		if ($price_check != $payment_data['amt']) {
		...
		if ($payment_data['state'] == 'ok' || $payment_data['state'] == 'test') {
			$order->order_status = $element->payment_params->verified_status;
		}

		if ($payment_data['state'] == 'fail') {
