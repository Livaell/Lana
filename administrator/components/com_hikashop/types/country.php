...
class hikashopCountryType{
	...
	function display($map, $value, $form = true, $options = 'class="inputbox" size="1"',$id=false){
		...
		foreach($zones as $oneZone){
			if(is_numeric($oneZone->zone_name_english)) $this->values[] = JHTML::_('select.option', $oneZone->zone_id, $oneZone->zone_name );
			else $this->values[] = JHTML::_('select.option', $oneZone->zone_id, $oneZone->zone_name_english.' ( '.$oneZone->zone_name.' )' );
		}
