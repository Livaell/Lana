...
class hikashopSubscriptionType{
	...
	function display($map,$values,$type='discount'){
		...
		$return = JHTML::_('hikaselect.radiolist',   $this->choice, "choice_".$map, 'onchange="updateSubscription(\''.$map.'\');"', 'value', 'text',$choiceValue);
		...
		foreach($this->groups as $oneGroup){
			$listAccess .= '<tr><td>';
			if(version_compare(JVERSION,'1.6.0','>=') || !in_array($oneGroup->value,array(29,30))) $listAccess .= '<input type="radio" onchange="updateSubscription(\''.$map.'\');" value="'.$oneGroup->value.'" '.(in_array($oneGroup->value,$valuesArray) ? 'checked' : '').' name="special_'.$map.'" id="special_'.$map.'_'.$oneGroup->value.'"/>';
