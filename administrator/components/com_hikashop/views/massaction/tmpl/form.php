...
<div id="hikabase_<?php echo $table->table; ?>_massactions" style="display:none">
	...
	<div id="<?php echo $table->table; ?>_actions_original">
		<?php echo JHTML::_('select.genericlist', $table->typevaluesActions, "action[".$table->table."][type][__num__]", 'class="inputbox chzn-done not-processed not-processed" size="1" onchange="updateMassAction(\'action\',\''.$table->table.'\',__num__); refreshSelect(\''.$table->table.'\',\'action\', __num__) "', 'value', 'text','action'.$table->table.'__num__');
...
<?php
				...
				<fieldset class="adminform" >
					<legend>'.JText::_( 'FILTERS' ).'</legend>
					<div id="all'.$table->table.'filters">';
				$count = 0;
				$key = 0;
		...
		echo		'</div><button class="btn" onclick="addHikaMassAction(\''.$table->table.'\',\'filter\'); refreshSelect(\''.$table->table.'\',\'filter\',-1) ;return false;">'.JText::_('ADD_FILTER').'</button>
				</fieldset>
				<fieldset class="adminform">
