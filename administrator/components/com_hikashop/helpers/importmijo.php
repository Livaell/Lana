...
class hikashopImportmijoHelper extends hikashopImportHelper
{
	...
	function importProducts()
	{
		...
		$this->db->setQuery('SELECT mjp.product_id, mjpi.image FROM `#__mijoshop_product` mjp '.
	...
	function importProductPrices()
	{
		...
		$this->db->setQuery('SELECT hkcur.currency_id FROM `#__mijoshop_currency` mjc '.
		...
		if (!empty($data))
		{
			$this->db->setQuery('INSERT IGNORE INTO `#__hikashop_price` (`price_product_id`,`price_value`,`price_currency_id`,`price_min_quantity`,`price_access`) '
					."SELECT hkprod.hk_Id, mjp.price, '".$data[0]->currency_id."', '0', 'all' "
	...
	function importOrders()
	{
		...
		$sql5 = 'UPDATE `#__hikashop_order` AS hko '.
			...
			"SET hko.order_payment_method = CONCAT('mijo import: ', mjo.payment_method) ".
